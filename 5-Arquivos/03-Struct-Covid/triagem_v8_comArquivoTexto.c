...
void popularVetor(){
    ...
    do {
        fscanf(procuradorArquivo,"%s %s %s %d %f %f %f %d %s", data, hora, vetor[i].nome, &vetor[i].idade,
            &vetor[i].peso, &vetor[i].altura, &vetor[i].imc, &vetor[i].resultadoTriagem, vetor[i].resultado);
        ...
        vetor[i].data.tm_mday = 26;
        vetor[i].data.tm_mon = 10 - 1;
        vetor[i].data.tm_year = 2020 - 1900;
        vetor[i].data.tm_hour = 20;
        vetor[i].data.tm_min = 05;
